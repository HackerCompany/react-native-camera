#import "RCTDepthCameraManager.h"
#import "RCTDepthCamera.h"
...
#import "RCTDepthSensorOrientationChecker.h"

@interface RCTDepthCameraManager ()

@property (strong, nonatomic) RCTDepthSensorOrientationChecker * sensorOrientationChecker;
@property (assign, nonatomic) NSInteger* flashMode;

@end

@implementation RCTDepthCameraManager
...
- (UIView *)view
{
  ...
  if(!self.camera){
    self.camera = [[RCTDepthCamera alloc] initWithManager:self bridge:self.bridge];
  }
  return self.camera;
}
...
RCT_CUSTOM_VIEW_PROPERTY(captureQuality, NSInteger, RCTDepthCamera) {
...
RCT_CUSTOM_VIEW_PROPERTY(aspect, NSInteger, RCTDepthCamera) {
...
RCT_CUSTOM_VIEW_PROPERTY(type, NSInteger, RCTDepthCamera) {
...
RCT_CUSTOM_VIEW_PROPERTY(flashMode, NSInteger, RCTDepthCamera) {
...
RCT_CUSTOM_VIEW_PROPERTY(torchMode, NSInteger, RCTDepthCamera) {
  dispatch_async(self.sessionQueue, ^{
...
RCT_CUSTOM_VIEW_PROPERTY(keepAwake, BOOL, RCTDepthCamera) {
  BOOL enabled = [RCTConvert BOOL:json];
  [UIApplication sharedApplication].idleTimerDisabled = enabled;
}

RCT_CUSTOM_VIEW_PROPERTY(mirrorImage, BOOL, RCTDepthCamera) {
  self.mirrorImage = [RCTConvert BOOL:json];
}

RCT_CUSTOM_VIEW_PROPERTY(mirrorVideo, BOOL, RCTDepthCamera) {
    self.mirrorVideo = [RCTConvert BOOL:json];
}

RCT_CUSTOM_VIEW_PROPERTY(cropToPreview, BOOL, RCTDepthCamera) {
    self.cropToPreview = [RCTConvert BOOL:json];
}

RCT_CUSTOM_VIEW_PROPERTY(barCodeTypes, NSArray, RCTDepthCamera) {
  self.barCodeTypes = [RCTConvert NSArray:json];
}

RCT_CUSTOM_VIEW_PROPERTY(captureAudio, BOOL, RCTDepthCamera) {
...
- (id)init {
  if ((self = [super init])) {
    ...
    self.sensorOrientationChecker = [RCTDepthSensorOrientationChecker new];
  }
  return self;
}
...
- (void)startSession {
  ...
  dispatch_async(self.sessionQueue, ^{
    ...
    __weak RCTDepthCameraManager *weakSelf = self;
    [self setRuntimeErrorHandlingObserver:[NSNotificationCenter.defaultCenter addObserverForName:AVCaptureSessionRuntimeErrorNotification object:self.session queue:nil usingBlock:^(NSNotification *note) {
      RCTDepthCameraManager *strongSelf = weakSelf;
